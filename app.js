...
// For deployment
const BASE = window.location.pathname.includes("/airbnb-visualisation-project/")
...
async function loadData() {
    try {
        ...
        state.citiesData = await d3.json(withBase('data/processed/cities_statistical_data.json'));
    ...
    state.housingPressureData = await d3.json(withBase('data/processed/housing_pressure.json'));
...
async function initializeAct2() {
    try {
        ...
        affordabilityData = await d3.json(withBase('data/processed/cities_affordability_2023.json'));
